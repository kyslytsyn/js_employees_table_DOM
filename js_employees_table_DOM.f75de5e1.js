...
function handleHeaderClick(index) {
    if (sortColumn === index) sortDir = sortDir === 'asc' ? 'desc' : 'asc';
    else {
        sortColumn = index;
        sortDir = 'asc';
    }
...
function sortRows(index) {
    rows.sort((rowA, rowB)=>{
        ...
        return sortDir === 'asc' ? cellA.localeCompare(cellB, undefined, {
            numeric: true
...
function addForm() {
    ...
    labels.forEach((el)=>{
        if (el.textContent === 'Position:') el.insertAdjacentElement('afterend', officeLabel);
    });
...
function validateForm(formData) {
    ...
    if (employeeName.length < 4) return 'Name must be at least 4 characters long.';
...
function cellEdit(e) {
    ...
    if (currentlyEditing) saveCellEdit(currentlyEditing);
...
//# sourceMappingURL=js_employees_table_DOM.f75de5e1.js.map
